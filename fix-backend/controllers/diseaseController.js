...
//   const aiRes = await axios.post(process.env.AI_MODEL_URL + '/predict', form);
//   return { key: aiRes.data.label, confidence: aiRes.data.confidence };
const mockPredict = (filename, fileSize) => {
  const diseases = Object.keys(DISEASE_DB);
  // Use file size + timestamp for variation (not filename which was always same)
  const seed = (fileSize || 0) + Date.now() % 1000;
  const idx = seed % diseases.length;
  const key = diseases[idx];
  return { key, confidence: Math.floor(78 + (seed % 20)) };
};
...
const analyzeImage = async (req, res, next) => {
  try {
    ...
    */
    const { key, confidence } = mockPredict(filename, req.file.size);
